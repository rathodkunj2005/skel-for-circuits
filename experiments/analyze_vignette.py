...
CIRCUIT_ANALYSIS_PROMPT_TEMPLATE = """\
...
MODEL_ONLY_ANALYSIS_LABEL = "Model-Only Analysis"
CIRCUIT_ANALYSIS_LABEL = "Circuit-Augmented Analysis"
...
def write_report(
    ...
    total_elapsed_s = sum(analysis["elapsed_s"] for analysis in analyses)
    ...
    lines = [
        ...
        f"**Slug:** `{slug}`  ",
        f"**Elapsed:** {total_elapsed_s:.1f}s total  ",
        f"",
        f"---",
        f"",
        f"## Circuit Prompt",
...
def main():
    ...
    model_only_prompt = MODEL_ONLY_PROMPT_TEMPLATE.format(
        scan=scan,
        prompt=prompt_text,
        task_description=task_description,
        counterfactual=counterfactual,
    )
    circuit_prompt = CIRCUIT_ANALYSIS_PROMPT_TEMPLATE.format(
        scan=scan,
        prompt=prompt_text,
        task_description=task_description,
        counterfactual=counterfactual,
    ...
    analyses = []
    for label, prompt in [
        (MODEL_ONLY_ANALYSIS_LABEL, model_only_prompt),
        (CIRCUIT_ANALYSIS_LABEL, circuit_prompt),
    ]:
        print(f"Calling {args.model} for {label.lower()}...")
        t0 = time.time()
        response = call_analysis_llm(prompt, model=args.model, api_key=api_key)
