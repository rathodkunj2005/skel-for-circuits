...
def create_alternate_world() -> list[tuple[str, str]]:
    countries = [country for country, _ in CURRENCIES]
    random.shuffle(countries)

    currencies = [currency for _, currency in CURRENCIES]
    random.shuffle(currencies)

    return list(zip(countries, currencies))


def create_alternate_world_prompt(world: list[tuple[str, str]]) -> str:
    prompt = "In this world,"
    for country, currency in world[:-1]:
        prompt += f" {country}'s currency is the {currency},"
    prompt += f" and {world[-1][0]}'s currency is the {world[-1][1]}.\n"
    return prompt


def create_analogy_prompt(world: list[tuple[str, str]]) -> tuple[str, str]:
    prompt = create_alternate_world_prompt(world)
    (country1, currency1), (country2, currency2) = random.sample(world, 2)
    prompt += f"{country1}:{currency1} :: {country2}:"
    return prompt, currency2
...
def _top_completion(
    prompt: str, tokenizer, model, max_new_tokens: int = 3
) -> str:
    prompt_ids = tokenizer(prompt, add_special_tokens=False, return_tensors="pt")[
        "input_ids"
    ].to(model.device)
...
def _score_candidates(prompt: str, tokenizer, model) -> dict[str, float]:
    return {
        currency: _aggregate_variant_scores(prompt, currency, tokenizer, model)
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Evaluate alternate-world analogy prompts with candidate-normalized answer probabilities."
    )
    parser.add_argument(
        "--num_trials",
        type=int,
    ...
    parser.add_argument(
        "--output_csv",
        type=Path,
        default=Path("data/evaluation_results/currency_analogy_results.csv"),
...
def main():
    ...
    for _ in range(args.num_trials):
        ...
        prompt, correct_answer = create_analogy_prompt(world)
